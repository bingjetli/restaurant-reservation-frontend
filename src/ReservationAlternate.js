...
export default function({item, selected, onSelect}){
    ...
    return (<TouchableHighlight 
        style={selected === item._id ? [reservation_styles.reservationMainView, reservation_styles.reservationAlternateView, {borderColor:appColors.main3, borderWidth:1, borderBottomColor:appColors.main3}] : [reservation_styles.reservationMainView, reservation_styles.reservationAlternateView]} 
        activeOpacity={0.6}
        ...
        <>
            ...
            <View style={reservation_styles.reservationHeaderView}>
                <Text style={[global_styles.bodyText, reservation_styles.reservationBodyText, {color:appColors.main4}]} >{item.phoneNumber}</Text>
                <Text style={[global_styles.bodyText, reservation_styles.reservationBodyText, {color:appColors.main4}]} >{item.seats} Guests</Text>
            </View>
            <View style={reservation_styles.reservationBodyView}>
                {item.tags && <TagViewer tags={item.tags} highlighted={true} />}
            </View>
            <View style={reservation_styles.reservationBodyView}>
                {item.notes && <Text style={[global_styles.bodyCaption, reservation_styles.reservationNotesText, {color:appColors.main4}]} >“{item.notes}”</Text>}
            </View>
        </>
    </TouchableHighlight>);
}